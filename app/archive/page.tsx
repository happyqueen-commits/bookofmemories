...
export default async function ArchivePage({ searchParams }: { searchParams: { q?: string; type?: string } }) {
  ...
  const items = await prisma.archiveMaterial.findMany({ where: { moderationStatus: ModerationStatus.approved, ...(q ? { title: { contains: q, mode: "insensitive" } } : {}), ...(type ? { materialType: type } : {}) }, orderBy: { publishedAt: "desc" } });
