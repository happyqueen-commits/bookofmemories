...
export async function submitMaterialAction(formData: FormData) {
  ...
  const parsed = personSchema.safeParse({
    ...
    militaryRank: formData.get("militaryRank") ?? formData.get("faculty"),
    serviceBranch: formData.get("serviceBranch") ?? formData.get("department"),
  ...
  if (!parsed.success || !contactParsed.success) {
    ...
    const params = new URLSearchParams({
      ...
      militaryRank: String(formData.get("militaryRank") ?? formData.get("faculty") ?? ""),
      serviceBranch: String(formData.get("serviceBranch") ?? formData.get("department") ?? ""),
      participationPeriod: String(formData.get("participationPeriod") ?? ""),
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const createdPerson = await tx.person.create({
      data: {
        ...
        faculty: payload.militaryRank ?? payload.faculty,
        department: payload.serviceBranch ?? payload.department,
        participationPeriod: payload.participationPeriod,
